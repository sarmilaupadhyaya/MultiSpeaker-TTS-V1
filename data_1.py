...
class TextMelMultispeakerDataset(torch.utils.data.Dataset):
    ...
    def get_pair(self, filepath_and_text):
        ...
        lid = self.get_lid(langid)
        return (text, mel, sid, eid, lid)

    def get_mel(self, filepath):
        #mel = np.load(filepath, allow_pickle=True).all()['melspec_feat']
    ...
    def get_text(self, text, add_blank=True):
        ...
        if self.add_blank:
            text_norm = intersperse(text_norm, int(200))  # add a blank token, whose id number is len(symbols)
        text_norm = torch.IntTensor(text_norm)
        return text_norm

    def __getitem__(self, index):
        text, mel, sid, eid, lid = self.get_pair(self.filepaths_and_text[index])
        item = {'y': mel, 'x': text, 'sid': sid, 'eid': eid, "lid":lid}
        return item
...
class TextMelMultispeakerBatchCollate(object):
    def __call__(self, batch):
        ...
        lid = torch.zeros((B, 1), dtype=torch.long)
        y_lengths, x_lengths = [], []

        for i, item in enumerate(batch):
            y_, x_, sid_, eid_, lid_ = item['y'], item['x'], item['sid'], item['eid'], item["lid"]
            y_lengths.append(y_.shape[-1])
            x_lengths.append(x_.shape[-1])
            y[i, :, :y_.shape[-1]] = torch.IntTensor(y_)
            x[i, :x_.shape[-1]] = x_
            sid[i,:1] = sid_
            eid[i,:1] = eid_
            lid[i,:1] = lid_
