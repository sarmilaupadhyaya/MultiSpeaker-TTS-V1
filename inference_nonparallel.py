...
outpath = '/home/ajkulkarni/workplace/subset2/Grad_TTS/nonparallel_v1'
...
if __name__ == '__main__':
    ...
    #parser.add_argument('-s', '--speaker', type=str, required=False, help='path to a file with texts to synthesize')
    #parser.add_argument('-e', '--emotion', type=str, required=False, help='path to a file with texts to synthesize')
    ...
    emotion_dict = {'1': 'sad','5': 'joy', '6': 'anger'}     
    with torch.no_grad():
        for emotion_id in ['1','5','6']:
            for i, text in enumerate(texts):
                emotion_tag = emotion_dict[emotion_id]
                if os.path.exists(os.path.join(outpath,'siwis'+'_'+emotion_tag+'_'+fnames[i])) == False:
                    print(f'Synthesizing {i} text...', end=' ')
                    x = torch.LongTensor(get_text(text)).unsqueeze(0).cuda()
                    x_lengths = torch.LongTensor([x.shape[-1]]).cuda()
                    g1 = torch.LongTensor(get_id(int('1'))).unsqueeze(0).cuda()
                    g2 = torch.LongTensor(get_id(int(emotion_id))).unsqueeze(0).cuda()
                    ...
                    write(os.path.join(outpath,'siwis'+'_'+emotion_tag+'_'+fnames[i]), 22050, audio)
