...
class TextEncoder(BaseModule):
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, 
                 ...
                 p_dropout, window_size=None, gin_channels1=0, gin_channels2=0):
        ...
        self.proj_w = DurationPredictor(n_channels + gin_channels1 + gin_channels2, filter_channels_dp, 
                                        kernel_size, p_dropout)

    def forward(self, x, x_lengths, g1=None, g2=None):
        ...
        if g1 is not None or g2 is not None:
            #print(x.shape, g1.shape, g2.shape)        
            g_exp1 = g1.expand(-1, -1, x.size(-1))
            g_exp2 = g2.expand(-1, -1, x.size(-1))
            #print(x.shape, g_exp.shape)    
            x_dp = torch.cat([torch.detach(x), g_exp1, g_exp2], 1)  
        else:
            x_dp = torch.detach(x)         
