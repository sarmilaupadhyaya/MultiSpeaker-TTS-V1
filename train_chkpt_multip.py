...
language_representation = args.language
...
log_dir = params.log_dir.rstrip('/')
#### add new folder in log_dir if doesn't exist -- for this version of the model
#### then all logs and checkpoints will be there
new_folder = f'/speaker_{speaker_representation}_lang_{language_representation}'
if not os.path.exists(log_dir + new_folder):
    os.makedirs(log_dir + new_folder)
log_dir = log_dir + new_folder
...
gin_channels_langs = params.gin_channels_langs
...
def run_train(rank, n_gpus):
    ...
    #############################
    # here we give two additional arguments to the model that determine its version by
    # the combination of speaker and language parameters: id or embedding
    model = GradTTS(nsymbols, n_enc_channels, filter_channels, filter_channels_dp,
                    n_heads, n_enc_layers, enc_kernel, enc_dropout, window_size,
                    n_feats, dec_dim, beta_min, beta_max, pe_scale, n_speakers,
                    n_langs, gin_channels_spk, gin_channels_langs,
                    speaker_representation, language_representation).cuda(rank)
    #############################
        ...
        if rank == 0:
            ...
            with tqdm(loader, total=len(train_dataset)//batch_size) as progress_bar:
                for batch_idx, batch in enumerate(progress_bar):
                    ...
                    ###################################
                    # define what will represent speaker and language:
                    # either id from the training file or npy file
                    if speaker_representation == 'id':
                        g1 = batch['sid'].cuda(rank)
                        #g1 = batch['sid']
                    else:  # represent speaker with embedding
                        g1 = batch['y'].clone().cuda(rank)

                    if language_representation == 'id':
                        g2 = batch['lid'].cuda(rank)
                    else:
                        g2 = batch['y'].clone().cuda(rank)
                        #g2 = batch['y'].clone().cuda()
                    ###################################
